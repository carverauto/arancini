...
The easiest way to use Arancini with Docker. This command will output the help message and exit:

```bash
docker run ghcr.io/nxthdr/arancini:main --help
```

To run Arancini with Docker with the default parameters, you can use the following command:
...
By default, Arancini listens on port `4000` for BMP messages.
...
Arancini maintains a state representing connected routers and their associated BGP peers and announced prefixes. This state is dumped to a file at specified intervals.
...
When the collector restarts, Arancini infers any missing withdraws from the initial Peer Up sequence, ensuring the database remains accurate despite downtime. However, any announcements received after the last saved state may be replayed. In short, Arancini guarantees a consistent database state, though it may contain some duplicate announcements following a restart.

Conversely, Arancini can be configured to stream updates to the event pipeline as is by disabling the state usage.

## Decoupling Collection from Curation

The Arancini application couples collection (parsing BMP messages) and curation (deduplication, synthetic withdraws) together for simplicity. Of course, you can disable curation to use the app as a stateless collector.
