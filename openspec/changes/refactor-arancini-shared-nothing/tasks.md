...
- [ ] 0.1 Remove `.await` while holding curation state locks in all paths (`process_updates`, peer down handling, peer-up recovery handling).
- [ ] 0.2 Refactor lock usage to gather emit sets while locked, drop lock, then perform channel sends/network I/O.
- [ ] 0.3 Refactor state dump flow so filesystem writes and `fsync` do not run while holding the state lock.
- [ ] 0.4 Replace string-based IPv4-to-IPv6 conversion (`format!` + `parse`) with numeric conversion.
- [ ] 0.5 Rewrite BMP framing path to reusable buffers (no per-message `Vec` allocation and no peek-loop spin on partial headers).
- [ ] 0.6 Remove constant Kafka partition key behavior in legacy path and define explicit partitioning/ordering strategy until JetStream cutover is complete.
- [ ] 0.7 Add bounded in-flight publish handling in legacy producer path (no strict one-batch-at-a-time pipeline).
- [ ] 0.8 Reduce avoidable update cloning; add shared attribute ownership where one BGP update yields many NLRI records.
- [ ] 0.9 Evaluate and implement faster hashing strategy for trusted state hot paths (or document rejection rationale if not adopted).
- [ ] 0.10 Reduce high-frequency metrics allocation/cardinality overhead in per-update paths.

## 1. Arancini Runtime and Messaging Implementation
- [ ] 1.1 Add monoio thread-per-core BMP listener workers and configure `SO_REUSEPORT` on listener sockets.
- [ ] 1.2 Integrate `io_uring` fixed-buffer (`FixedSizeSlot`) packet ingress and zero-copy parse handoff.
- [ ] 1.3 Ensure deterministic session-to-core ownership for BMP connections and prohibit cross-core RIB mutation.
- [ ] 1.4 Build bounded lock-free `kanal` MPSC bridge from monoio workers to a dedicated Tokio NATS sidecar thread.
...
- [ ] 1.10 Apply required socket options (`SO_REUSEPORT`, `TCP_NODELAY`) in runtime socket setup.
- [ ] 1.13 Add runtime-configurable listener backlog and ingest `SO_RCVBUF` controls; align defaults with documented host tuning.
