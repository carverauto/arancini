...
pub async fn dump<T: StateStore + Serialize>(
    state: AsyncState<T>,
    cfg: CurationConfig,
) -> Result<()> {
    ...
    match encoded {
        ...
        Err(e) => {
            anyhow::bail!("Failed to serialize state using postcard: {}", e);
        }
    }
}
...
pub async fn load<T: StateStore + for<'de> Deserialize<'de>>(
    ...
    match tokio::fs::read(&cfg.state_path).await {
        Ok(encoded) => match postcard::from_bytes::<T>(&encoded) {
            Ok(store) => {
            ...
            Err(e) => {
                error!(
                    "Failed to deserialize curation state from {} using postcard: {}",
                    cfg.state_path, e
                );
                let backup_path =
                    format!("{}.corrupted-{}", cfg.state_path, Utc::now().timestamp());
        ...
        Err(e) => {
            if e.kind() == std::io::ErrorKind::NotFound {
                ...
                error!(
                    "Failed to read curation state file {}: {}",
                    cfg.state_path, e
                );
            }
        }
